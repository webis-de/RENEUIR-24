...
# is included in sentence_transformers.cross_encoder.evaluation
from CERerankingEvaluator_bm25cat import CERerankingEvaluator
from CrossEncoderEarlyStopping import CrossEncoder
...
train_batch_size = MODELS[SLURM_TASK_ID_TO_MODEL[int(os.environ.get('SLURM_ARRAY_TASK_ID'))]][0]
learning_rate = MODELS[SLURM_TASK_ID_TO_MODEL[int(os.environ.get('SLURM_ARRAY_TASK_ID'))]][1]
num_epochs = 1
model_save_path = data_folder + '/../models-bm25-cat/train-cross-encoder-kd-bm25cat-' + \
...
with open(collection_filepath, 'r', encoding='utf8') as fIn:
    for line in tqdm.tqdm(fIn, 'read collection'):
...
with open(queries_filepath, 'r', encoding='utf8') as fIn:
    for line in tqdm.tqdm(fIn, 'read queries'):
...
with gzip.open(train_eval_filepath, 'rt') as fIn:
    for line in tqdm.tqdm(fIn, 'read triples'):
...
with open(teacher_logits_filepath) as fIn:
    for line in tqdm.tqdm(fIn, 'read logits'):
        pos_score, neg_score, qid, pid1, pid2 = line.strip().split("\t")

        if qid in dev_qids:  # Skip queries in our dev dataset
            continue

        train_samples.append(InputExample(texts=["{} [SEP] {}".format(
            scores[qid][pid1], queries[qid]), corpus[pid1]], label=float(pos_score)))
        train_samples.append(InputExample(texts=["{} [SEP] {}".format(
            scores[qid][pid2], queries[qid]), corpus[pid2]], label=float(neg_score)))

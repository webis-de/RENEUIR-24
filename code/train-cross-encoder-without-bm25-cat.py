...
train_batch_size = 32
num_epochs = 1
model_save_path = data_folder + '/../models-bm25-cat/train-cross-encoder-without-kd-bm25cat-'+model_name.replace("/", "-")+'-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
...
with gzip.open(train_eval_filepath, 'rt') as fIn:
    for line in tqdm.tqdm(fIn, 'read triples'):
        ...
        if qid in dev_samples:
            ...
            dev_samples[qid]['query'].append(queries[qid])
